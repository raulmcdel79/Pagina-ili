...
import { whatsappUrl, WHATSAPP_NUMBER, WHATSAPP_DEFAULT_MESSAGE, MAILTO_ADDRESS, MAILTO_SUBJECT, MAILTO_BODY_TEMPLATE, CTA_WHATSAPP_LABEL, CTA_EMAIL_LABEL } from '../constants/contact';
...
const DueloAnimal: React.FC = () => {
  return (
    <>
      <section
        id="duelo"
        className="relative w-full py-24 md:py-40 bg-cover bg-center overflow-hidden text-brand-light"
        style={{ backgroundImage: "url('/imagenes/muerte-duelo-animal-perdida-mascota.png')", scrollMarginTop: 'var(--header-offset)' }}
      >
        <div className="absolute inset-0 bg-gradient-to-b from-[#7B4A1D]/90 via-[#7B4A1D]/70 to-black/60"></div>
  ...
  <div className="text-base text-brand-light/80">RECOMENDADO: Pack de 4 sesiones (1h/semana) 130 €<br/><span className="text-xs">(posibilidad de 2h/semana si lo necesitas)</span></div>
          </div>
        </div>
    ...
    <div className="max-w-3xl mx-auto px-4">
      ...
      <div className="mt-8 flex justify-center gap-4">
        <a
          href={whatsappUrl(WHATSAPP_NUMBER, WHATSAPP_DEFAULT_MESSAGE)}
          target="_blank"
          rel="noopener noreferrer"
          className="px-6 py-3 rounded bg-green-500 text-white font-semibold text-lg shadow hover:bg-green-600 transition"
        >
          {CTA_WHATSAPP_LABEL}
        </a>

        <a
          href={`mailto:${MAILTO_ADDRESS}?subject=${encodeURIComponent(MAILTO_SUBJECT)}&body=${encodeURIComponent(MAILTO_BODY_TEMPLATE)}`}
